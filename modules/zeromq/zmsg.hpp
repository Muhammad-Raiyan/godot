...
class zmsg
{
  public:
    ...
    zmsg(char const *body) { body_set(body); }
    ...
    zmsg(zmq::socket_t &socket) { recv(socket); }
    ...
    zmsg(zmsg &msg)
    {
        ...
        std::copy(msg.m_part_data.begin(), msg.m_part_data.end(),
                  m_part_data.begin());
    }
    ...
    bool recv(zmq::socket_t &socket)
    {
        ...
        while (1) {
            ...
            if (message.size() == 17 && ((unsigned char *) message.data())[0] == 0) {
                ...
                m_part_data.push_back(
                  ustring((unsigned char *) message.data(), message.size()));
            }
            if (!message.more()) {
                break;
            }
    ...
    void send(zmq::socket_t &socket)
    {
        for (size_t part_nbr = 0; part_nbr < m_part_data.size(); part_nbr++) {
            ...
            try {
                socket.send(message,
                            part_nbr < m_part_data.size() - 1 ? ZMQ_SNDMORE : 0);
            }
            catch (zmq::error_t error) {
                assert(error.num() != 0);
            }
    ...
    size_t parts() { return m_part_data.size(); }
    ...
    // zmsg_append
    void push_back(char *part) { m_part_data.push_back((unsigned char *) part); }
